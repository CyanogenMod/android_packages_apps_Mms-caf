...
public class MessageListItem extends ZoomMessageListItem implements
    ...
    public void bind(MessageItem msgItem, int accentColor,
            boolean convHasMultiRecipients, int position, ListView listView, boolean isIncoming) {
        if (DEBUG) {
                ...
                mMessageItem.setOnPduLoaded(new MessageItem.PduLoadedCallback() {
                    public void onPduLoaded(MessageItem messageItem) {
                        ...
                        if (messageItem != null && mMessageItem != null &&
                            ...
                            int totalHeight = 0;
                            for (MediaModel s : messageItem.getSlideshow()) {
                                if (s instanceof RegionMediaModel) {
                                    int regionHeight = ((RegionMediaModel) s).getRegion().getHeight();
                                    totalHeight += Math.min(ThumbnailManager.THUMBNAIL_TARGET_SIZE, regionHeight);
                                }
                            }
                            View view = mMmsView;
                            if (view == null) {
                                view = findViewById(R.id.message_attachment_stub);
                            }
