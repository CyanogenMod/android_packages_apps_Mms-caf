...
public class MessageListItem extends ZoomMessageListItem implements
    ...
    public void bind(MessageItem msgItem, int accentColor, boolean convHasMultiRecipients,
                     int position, ListView listView, boolean isIncoming, MessageInfoCache messageInfoCache) {
        if (DEBUG) {
        ...
        mListView = listView;
        mMessageInfoCache = messageInfoCache;
    ...
    public void unbind() {
        ...
        if (mMmsView != null) {
            mMmsView.removeAllViews();
            mMmsView.removeAllViewsInLayout();
        }
    }
    ...
    private void bindCommonMessage(final boolean sameItem) {
        ...
        if (mMessageItem.isSms() ||
                (mMessageItem.mAttachmentType == WorkingMessage.TEXT && haveLoadedPdu)) {
            showMmsView(false);
            ...
            if (mMessageItem.mSlideshow == null) {
                ...
                if (mPresenter != null) {
                    ...
                    presenterOptions.setCallback(mItemCallback);
                    mPresenter.present(mMmsView, presenterOptions);
                }
            }
        }
    ...
    private Presenter.PresenterOptions.Callback mItemCallback = new Presenter.PresenterOptions.Callback() {
        @Override
        ...
        public long getMessageId() {
            return getMessageItem().getMessageId();
        }

        @Override
        public void donePresenting(int height) {
            mMessageInfoCache.addMessageItemInfo(getMessageId(), height);
        }
    };
    ...
    public void showMmsView(boolean visible) {
        if (mMmsView == null) {
        ...
        if (mMmsView != null) {
            ...
            if (mMessageItem.mSlideshow == null) {
                int totalHeight = mMessageInfoCache.getCachedTotalHeight(getMessageItem().getMessageId());
