            ...
            handler.post(new Runnable() {
                ...
                public void run() {
                    for (TextView textView : mZoomableTextViewList) {
                        // Set the newly added TextView's font to the current size.
                        CharSequence currentText = textView.getText();
                        SpannableStringBuilder builder = new SpannableStringBuilder(currentText);
                        builder.removeSpan(AbsoluteSizeSpan.class);
                        builder.setSpan(new AbsoluteSizeSpan(mZoomFontSize, true), 0, builder.length(), 0);
