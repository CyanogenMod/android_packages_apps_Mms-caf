...
public class MessageInfoCache {
    ...
    public int getCachedTotalHeight(long messageId) {
        ...
        if (mimeTypes != null) {
            for (String mimeType : mimeTypes) {
                if (mimeType.startsWith("image")) {
                    ...
                    totalHeight += getTextHeight(text, 500);
                } else {
                    totalHeight += SimpleAttachmentPresenter.getStaticHeight();
                }
            }
        }
        return totalHeight;
    ...
    public String[] getCachedMimeTypes(long msgId) {
        ...
        if (mMessagePartsCursor != null && mMessageIdPositionMap.containsKey(msgId)) {
            ...
            mMessagePartsCursor.moveToPosition(cursorPosition);
            StringBuilder stringBuilder = new StringBuilder();
            do {
    ...
    private int getTextHeight(String text, int maxWidth) {
        ...
        while(index < length - 1) {
            index += mPaint.breakText(text, index, length, true, maxWidth, null);
            lineCount++;
        }
    ...
    public void addMessageItemInfo(long messageId, int height) {
        System.out.println(messageId + " -> " + height);
