...
public class WorkingMessage {
    ...
    public int setAttachment(int type, Uri dataUri, boolean append) {
        return setAttachment(type, dataUri, append, ThumbnailManager.THUMBNAIL_SIZE);
    }
     ...
     */
    public int setAttachment(int type, Uri dataUri, boolean append, int maxWidth) {
        if (Log.isLoggable(LogTag.APP, Log.VERBOSE)) {
        ...
        if (mSlideshow != null && type == IMAGE) {
            ...
            if (numSlides > 0) {
                ...
                if (imgModel instanceof ImageModel) {
                    ...
                    ((ImageModel) imgModel).loadThumbnailBitmap(null, maxWidth);
                }
            }
        }
    ...
    private void syncTextToSlideshow() {
        Thread.dumpStack();
        System.out.println("syncTextToSlideshow");
        if (mSlideshow == null) {
            return;
        }
        System.out.println("syncTextToSlideshow " + mSlideshow.size());
        TextModel textModel;
        if (mSlideshow.get(0) instanceof TextModel) {
            textModel = (TextModel) mSlideshow.get(0);
        } else {
             textModel = new TextModel(mActivity, ContentType.TEXT_PLAIN, "text_0.txt",
                    mSlideshow.getLayout().getTextRegion());
            mSlideshow.add(0, textModel);
        }
        textModel.setText(mText);
    }
