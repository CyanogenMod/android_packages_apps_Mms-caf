...
public class WorkingMessage {
    ...
    private boolean loadFromUri(Uri uri) {
        ...
        System.out.println("loadFromUri");
    ...
    public int setAttachment(int type, Uri dataUri, boolean append) {
        ...
        System.out.println("Third " + mSlideshow.size());
        if (mSlideshow != null && type == IMAGE) {
            ...
            if (numSlides > 0) {
                ...
                if (imgModel instanceof ImageModel) {
                    ...
                    ((ImageModel) imgModel).loadThumbnailBitmap(null);
                }
            }
        }
    ...
    public void syncTextFromSlideshow() {
        System.out.println("syncTextFromSlideshow1");
        // Don't sync text for real slideshows.
        if (mSlideshow == null || mSlideshow.size() == 0) {
            return;
        }
        System.out.println("syncTextFromSlideshow2 " + mSlideshow.size());
        MediaModel slide = mSlideshow.get(0);
        if (slide == null || !(slide instanceof TextModel)) {
            return;
        }
        System.out.println("syncTextFromSlideshow3 " + mSlideshow.size());
